...
export interface ScrollPortraitWallProps {
  /**
   * Big sticky title. Ignored if `titleImageSrc` is provided — the image
   * takes over as the sticky centerpiece and gets the same
   * `mix-blend-exclusion` treatment.
   */
  title?: React.ReactNode;
  /** Sticky image to use instead of text (e.g. a logo/wordmark PNG or SVG). */
  titleImageSrc?: string;
  /** Alt text for the title image (also used as the section aria-label). */
  titleImageAlt?: string;
  /** Optional intrinsic width/height hints for the title image. */
...
// Local avatars — put these files in /public/avatars/
const DEMO_SPEAKERS: Speaker[] = [
  ...
  ...s,
  // 5 local avatars, cycled across the speakers.
  src: `/avatars/k${(i % 5) + 1}.png`,
}));

export function ScrollPortraitWall({
  title = "KRAFT",
  titleImageSrc,
  titleImageAlt = "/avs-logo.png",
  titleImageWidth,
  titleImageHeight,
  date = "Krafts Products",
  hint = "Scrol to see products",
  ...
  return (
    ...
    >
      ...
      {/* Sticky centred title — inverts against whatever portrait is behind it.
          Works identically for text or an image since mix-blend-exclusion
          applies to the element itself, not to its content type. */}
      <div className="pointer-events-none sticky top-1/2 z-20 -translate-y-1/2 text-center text-white mix-blend-exclusion">
        {titleImageSrc ? (
          ...
          <img
            ...
            draggable={false}
            className="mx-auto h-auto w-[70vw] max-w-2xl select-none object-contain sm:w-[50vw] md:w-[40vw] lg:w-[32vw]"
          />
        ) : (
                ...
                <div key={ci} className="aspect-square flex-1">
                  ...
                  >
                    ...
                    <img
                      ...
                      decoding="async"
                      draggable={false}
                      className="h-full w-full object-cover contrast-[1.15] filter transition-transform duration-500 ease-in-out hover:scale-95"
                    />
                    {showCaptions && (
